...
public class ImportFromDiagramAction extends Action {
	...
	private static class ImportFromDiagramWizard extends Wizard {
		...
		public boolean performFinish() {
			final TableModel[] tables = page.getSelectedTableModel();
			final IFile file = page.getSelectedFile();
			
			stack.execute(new Command(){
				@Override public void execute() {
					for(TableModel table: tables){
						TableModel oldTable = model.getTable(table.getTableName());
						
						stripConnections(tables, table.getModelSourceConnections());
						stripConnections(tables, table.getModelTargetConnections());
						
                        if(oldTable != null){
                            model.removeChild(oldTable);
				...
				private void stripConnections(TableModel[] importModels,
						java.util.List<AbstractDBConnectionModel> conns){
